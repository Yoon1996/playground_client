...
const CalendarComponent = () => {
    ...
    const renderDotw = () => {
        return dotw.map((dotw, index) => (
            <div key={index} className="flex justify-center w-8">
                {dotw}
            </div>
        ));
    };
    ...
    const renderCalendar = () => {
        ...
        return days.map((day, index) => (
            <div
                ...
                className="flex justify-center items-center w-8 h-8 hover:bg-primary-light hover:rounded-md cursor-pointer"
                key={index}
            >
                {day.getDate()}
            </div>
    ...
    return (
        <>
            <div className="w-full flex flex-col gap-4 justify-center items-center">
                ...
                <div className="flex w-full justify-between gap-6">{renderDotw()}</div>
                <div className="flex w-full justify-between gap-6">{renderCalendar()}</div>
            </div>
        </>
    );
};
